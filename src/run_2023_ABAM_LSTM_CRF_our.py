...
from utils import InputFeatures_head_tail, get_data_with_labels2

from models import TokenBERT_abam_LSTM_CRF_our
...
def training(train_dataloader, model, device, optimizer, scheduler, max_grad_norm):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        batch_input_ids, batch_input_mask, batch_sentence_ids, batch_sent_length, batch_char_ids, batch_char_len, batch_pos_ids, batch_pieces2word, batch_graph, batch_label_mask, batch_head, batch_tail, batch_as_mask, batch_am_mask, batch_AM_label_ids, batch_AS_label_ids = batch
        # forward pass
        loss1 = model(
...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    for step, batch in enumerate(sample_dataloader):
        batch = tuple(t.to(device) for t in batch)
        batch_input_ids, batch_input_mask, batch_sentence_ids, batch_sent_length, batch_char_ids, batch_char_len, batch_pos_ids, batch_pieces2word, batch_graph, batch_label_mask, batch_as_mask, batch_head, batch_tail, batch_am_mask, batch_AM_label_ids, batch_AS_label_ids = batch
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='../models_final_abam_gatlstm_our/',
...
#词性
    ...
    for count, AD in AURC_DATA_dict.items():
        ...
        if "1" not in AD['tokenized_sentence_spacy_as_labels']:
            ...
            as_label_mask = [0 if label[0] != "0" else -np.inf for label in AD['tokenized_sentence_spacy_as_labels'].split(' ')]
        as_label_mask = as_label_mask[:args.max_spacy_length] + [-np.inf] * max(0, args.max_spacy_length - len(as_label_mask))
    ...
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sent_length, all_char_ids, all_char_len, all_pos_ids, all_pieces2word, all_graph, all_label_mask, all_head_label, all_tail_label, all_as_mask, all_am_mask, all_AM_label_ids, all_AS_label_ids)
    ...
    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sent_length, all_char_ids, all_char_len, all_pos_ids, all_pieces2word, all_graph, all_label_mask, all_head_label, all_tail_label, all_as_mask, all_am_mask, all_AM_label_ids, all_AS_label_ids)
    ...
    test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sent_length, all_char_ids, all_char_len, all_pos_ids, all_pieces2word, all_graph, all_label_mask, all_head_label, all_tail_label, all_as_mask, all_am_mask, all_AM_label_ids, all_AS_label_ids)
    ...
    if args.train:
        ...
        # Model
        model = TokenBERT_abam_LSTM_CRF_our(
