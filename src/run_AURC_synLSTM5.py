...
from models import TokenBERT_synLSTM5
...
def training(train_dataloader, model, device, optimizer, scheduler, max_grad_norm):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        loss2, logits_AM2 = model(
            ...
            AM_labels=batch_AM_label_ids,
            AS_labels=batch_AS_label_ids,
            term2span=batch_as_mask,
        )
        a = batch_as_mask.cpu().numpy()
...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    for step, batch in enumerate(sample_dataloader):
        ...
        with torch.no_grad():
            ...
            for i in AS_tags1:
                if len(set(i)) != 1:
                    as_label_mask = [0 if label != 0 else -np.inf for label in i]
                    AS_tags1s.append(as_label_mask[:max_spacy_length] + [0] * max(0, max_spacy_length - len(as_label_mask)))
                else:
                    AS_tags1s.append(i[:max_spacy_length] + [0] * max(0, max_spacy_length - len(i)))
            AS_tags1s_t = torch.tensor([f for f in AS_tags1s], dtype=torch.float).to(device)
            #print(AS_tags1s)


            AM_tags2, logits_AM2 = model(
                model='term2span',
                input_ids=batch_input_ids,
                token_type_ids=batch_sentence_ids,
                attention_mask=batch_input_mask,
                ...
                graph=batch_graph,
                label_mask=batch_label_mask,
                term2span=AS_tags1s_t
            )
            # batch_am_mask
            AM_tags1s = []
            for i in AM_tags1:
                if len(set(i)) != 1:
                    am_label_mask = [0 if label != 0 else -np.inf for label in i]
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='../models_final5/',
