...
from utils import InputFeatures_char_word_pos, get_data_with_labels2

from models import TokenBERT_synLSTM3
...
def training(train_dataloader, model, device, optimizer, scheduler, max_grad_norm):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        batch_input_ids, batch_input_mask, batch_sentence_ids, batch_sent_length, batch_char_ids, batch_char_len, batch_pos_ids, batch_pieces2word, batch_graph, batch_label_mask, batch_AM_label_ids, batch_AS_label_ids = batch
...
#词性
    ...
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sent_length, all_char_ids, all_char_len, all_pos_ids, all_pieces2word, all_graph, all_label_mask, all_AM_label_ids, all_AS_label_ids)
    ...
    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sent_length, all_char_ids, all_char_len, all_pos_ids, all_pieces2word, all_graph, all_label_mask, all_AM_label_ids, all_AS_label_ids)
    ...
    test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sent_length, all_char_ids, all_char_len, all_pos_ids, all_pieces2word, all_graph, all_label_mask, all_AM_label_ids, all_AS_label_ids)
    ...
    if args.train:
        ...
        for epoch in range(args.epochs):
            ...
            if micro_train > best_f1s_dev:
                best_f1s_dev = micro_train
                best_epoch = epoch
    ...
    if args.eval:
        ...
        y_true_test, y_pred_test, micro_test, macro_test = evaluation(
            test_dataloader, model=model, device=device, tokenizer=tokenizer)
        print("TEST:   micro_F1. %.4f |  macro_F1 %.4f" % (micro_dev, macro_dev))
