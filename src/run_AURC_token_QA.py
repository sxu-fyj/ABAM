...
from utils import InputFeatures_QA, get_data_with_labels
...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    for step, batch in enumerate(sample_dataloader):
        ...
        for input_ids, AM_label_ids, AS_label_ids, AM_tags, AS_tags in zip(batch_input_ids, batch_AM_label_ids,
...
#词性
    ...
    for count, AD in AURC_DATA_dict.items():
        ...
        input_labels = input_labels[:args.max_sequence_length] + [0] * max(0, args.max_sequence_length - len(input_labels))
        ...
        input_labels2 = input_labels2[:args.max_sequence_length] + [0] * max(0, args.max_sequence_length - len(input_labels2))
        sequence_dict['AS_label_ids'] = input_labels2
        sequence_dict['input_tokens'] = tokenizer.convert_ids_to_tokens(_bert_inputs2)
        pieces2word = pieces2words[int(count)]
        assert len(input_labels)==len(input_labels2)==args.max_sequence_length
        assert am_label_len==as_label_len==pieces2word.shape[0]
        assert bert_len==pieces2word.shape[1]
        ...
        if AD[args.target_domain] == 'Dev':
            eval_features += FE
            all_input_tokens_dev.append(tokenizer.convert_ids_to_tokens(_bert_inputs2))
        if AD[args.target_domain] == 'Test':
            test_features += FE
            all_input_tokens_test.append(tokenizer.convert_ids_to_tokens(_bert_inputs2))
    # TRAIN DATA
