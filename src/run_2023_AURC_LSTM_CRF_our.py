...
from utils import InputFeatures_AURC, get_data_with_labels2

from models import TokenBERT_LSTM_CRF_our



with open('../AURC/pieces2word.pkl', 'rb') as fr:
...
def training(train_dataloader, model, device, optimizer, scheduler, max_grad_norm):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        batch_input_ids, batch_input_mask, batch_sentence_ids, batch_sent_length, batch_pieces2word, batch_graph, batch_label_mask, batch_head, batch_tail, batch_am_mask, batch_AM_label_ids = batch
        # forward pass
        loss1 = model(
        #loss1, head_loss, tail_loss = model(
            model = 'embedding',
...
def evaluation(sample_dataloader, model, device, tokenizer):
    ...
    for step, batch in enumerate(sample_dataloader):
        ...
        for input_ids, AM_label_ids, AM_tags in zip(batch_label_mask, batch_AM_label_ids,
                                                                           batch_AM_tags):
            ...
            correct_label_ids = [l1 for l1 in zip(aaa[0:seq_len])]
            correct_tags = [l1 for l1 in zip(aaaa[0:seq_len])]
...
def main():
    ...
    parser.add_argument('--eval_batch_size', type=int, default=32, help='Your evaluation set batch size.')
    parser.add_argument('--test_batch_size', type=int, default=32, help='Your test set batch size.')
    parser.add_argument('--target_domain', type=str, default='in-domain', help='inner OR cross')
    parser.add_argument('--input_file', type=str, default='../AURC/data_dict_bert.json', help='The input dict file.')
    parser.add_argument('--data_dir', type=str, default='../AURC/', help='The data directory.')
    parser.add_argument('--output_dir', type=str, default='../models_final_gatlstm_our/',
...
#词性
    ...
    char_dict = {'I': 0, 'n': 1, 'o': 2, 'r': 3, 'd': 4, 'e': 5, 't': 6, 'i': 7, 's': 8, 'u': 9, 'a': 10, 'w': 11, 'm': 12, 'f': 13,
     ...
     '~': 86, '_': 87, 'X': 88, 'ā': 89, 'à': 90, '«': 91, '»': 92, '£': 93, '“': 94, '”': 95, '’': 96, '…': 97, '–':98, '´':99, '‘': 100, '`':101, '|':102, '•':103, '½':104, 'ﬂ':105, '°':107, '−':108, '®':109, '\\':110, '■':111, 'Â':112, 'padding': 106}
    ...
    for count, AD in AURC_DATA_dict.items():
        sequence_dict = tokenizer.encode_plus(AD['tokenized_sentence_spacy'], '<'+AD['topic']+'>', max_length=args.max_sequence_length,
        ...
        for boundary in BIEO_AM_label:
            if boundary[0] == 'S':
                ...
                heads.append(0)
                tails.append(0)
        heads_labels = heads[:args.max_spacy_length] + [0] * max(0, args.max_spacy_length - len(heads))
        sequence_dict['heads_label'] = heads_labels

        tails_labels = tails[:args.max_spacy_length] + [0] * max(0, args.max_spacy_length - len(tails))
        sequence_dict['tails_label'] = tails_labels
        ...
        label_mask = [1 for label in AD['tokenized_sentence_spacy_labels'].split(' ')]
        ...
        am_label_mask = [0 if label[0] != "n" else -np.inf for label in AD['tokenized_sentence_spacy_labels'].split(' ')]
        am_label_mask = am_label_mask[:args.max_spacy_length] + [-np.inf] * max(0, args.max_spacy_length - len(am_label_mask))
        sequence_dict['am_label_mask'] = am_label_mask  # 方面项的标签


        sent_length=len(AD['tokenized_sentence_spacy_labels'].split(' '))
        ...
        assert am_label_len == label_mask_len == pieces2word_.shape[0]
        assert len(input_am_labels) == args.max_spacy_length
        ...
        assert am_label_len == label_mask_len == graph_.shape[0]
        assert len(input_am_labels) == args.max_spacy_length
        ...
        # char
        # print(AD['tokenized_sentence_spacy'])
        ...
        FE = [
            InputFeatures_AURC(
                ...
                head_label=sequence_dict['heads_label'],
                tail_label=sequence_dict['tails_label'],
    ...
    all_head_label = torch.tensor([f.head_label for f in train_features], dtype=torch.long)
    all_tail_label = torch.tensor([f.tail_label for f in train_features], dtype=torch.long)
    ...
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sent_length, all_pieces2word, all_graph, all_label_mask, all_head_label, all_tail_label, all_am_mask, all_AM_label_ids)
    ...
    all_head_label = torch.tensor([f.head_label for f in eval_features], dtype=torch.long)
    all_tail_label = torch.tensor([f.tail_label for f in eval_features], dtype=torch.long)
    ...
    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sent_length, all_pieces2word, all_graph, all_label_mask, all_head_label, all_tail_label, all_am_mask, all_AM_label_ids)
    ...
    all_head_label = torch.tensor([f.head_label for f in test_features], dtype=torch.long)
    all_tail_label = torch.tensor([f.tail_label for f in test_features], dtype=torch.long)
    ...
    test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sent_length, all_pieces2word, all_graph, all_label_mask, all_head_label, all_tail_label, all_am_mask, all_AM_label_ids)
