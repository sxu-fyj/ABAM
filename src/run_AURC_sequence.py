...
from models import TokenBERT_sequence
...
def training(train_dataloader, model, device, optimizer, scheduler, max_grad_norm):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        batch_input_ids, batch_input_mask, batch_sentence_ids, batch_sent_length, batch_char_ids, batch_char_len, batch_pos_ids, batch_pieces2word, batch_graph, batch_label_mask, batch_as_mask, batch_am_mask, batch_AM_label_ids, batch_AS_label_ids = batch
        # forward pass
        loss1 = model(
            model = 'embedding',
...
#词性
    ...
    for count, AD in AURC_DATA_dict.items():
        sequence_dict = tokenizer.encode_plus(AD['tokenized_sentence_spacy']+'out', '<'+AD['topic']+'>', max_length=args.max_sequence_length,
        ...
        input_am_labels2 = []
        begin=0
        for i in range(len(BIEO_AM_label)):
            if BIEO_AM_label[i] != 'O':
                if "B-" in BIEO_AM_label[i]:
                    begin=i
                    input_am_labels2.append(i+1)
                if "I-" in BIEO_AM_label[i]:
                    input_am_labels2.append(i+1)
                if "E-" in BIEO_AM_label[i]:
                    input_am_labels2.append(begin)
            else:
                input_am_labels2.append(len(BIEO_AM_label))
        input_am_labels = input_am_labels2[:args.max_spacy_length] + [0] * max(0, args.max_spacy_length - len(input_am_labels2))
        ...
        input_as_labels2 = []
        begin = 0
        for i in range(len(BIEO_AS_label)):
            if BIEO_AS_label[i] != 'O':
                if "B-" in BIEO_AS_label[i]:
                    begin = i
                    input_as_labels2.append(i+1)
                if "I-" in BIEO_AS_label[i]:
                    input_as_labels2.append(i+1)
                if "E-" in BIEO_AS_label[i]:
                    input_as_labels2.append(begin)
            else:
                input_as_labels2.append(len(BIEO_AS_label))
        input_as_labels2 = input_as_labels2[:args.max_spacy_length] + [0] * max(0, args.max_spacy_length - len(input_as_labels2))
        sequence_dict['AS_label_ids'] = input_as_labels2
    ...
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sent_length, all_char_ids, all_char_len, all_pos_ids, all_pieces2word, all_graph, all_label_mask, all_as_mask, all_am_mask, all_AM_label_ids, all_AS_label_ids)
    ...
    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sent_length, all_char_ids, all_char_len, all_pos_ids, all_pieces2word, all_graph, all_label_mask, all_as_mask, all_am_mask, all_AM_label_ids, all_AS_label_ids)
    ...
    test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_sent_length, all_char_ids, all_char_len, all_pos_ids, all_pieces2word, all_graph, all_label_mask, all_as_mask, all_am_mask, all_AM_label_ids, all_AS_label_ids)
